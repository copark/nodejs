...
//	drawReplyGraph(data);
	...
	$.getJSON("request", 
						{ cmd: "topCoupon", condition: "buyQuantity"}, drawSaleGraph);
	$.getJSON("request", 
						{ cmd: "topCoupon", condition: "satisfactionAvg"}, drawPointGraph);
	$.getJSON("request", 
						{ cmd: "topCoupon", condition: "viewCount"}, drawViewGraph);
	$.getJSON("request", 
						{ cmd: "topCoupon", condition: "epilogueCount"}, drawReplyGraph);
	
	// 웹소켓 서버에 접속한다. 
	var socket = io("ws://localhost"); // url을 생략하면 현재 도메인 
	socket.on("new5", drawSaleGraph);
});

// 판매순 그래프를 그린다.(Canvas)
function drawSaleGraph(data){
	var context = $("#graph_by_sale")[0].getContext("2d");
	//var context2 = document.getElementById("graph_by_sale").getContext("2d");
	//var context3 = document.querySelector("graph_by_sale").getContext("2d");

	console.info("w = " + $(document).width()	+ ", h = " + $(document).height());
	// x, y 축 그리기	
	context.beginPath();
	...
	console.info("r = " + r + ", b q = " + data[0].buyQuantity);

	$.each(data, function(i){
		var x = (barW + gap) * i + gap + marginLeft;
